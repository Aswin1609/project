...
class login_page:
    def __init__(self, root):
        ...
        self.email_entry = Entry(
            self.frame3, font=("times new roman", 15, "bold"), bg="white", fg="gray"
        )
        ...
        self.login_button = Button(
            ...
            bg="blue",
            fg="white",
        ).place(x=50, y=200, width=300)

        self.forgotten_pass = Button(
            self.frame3,
    ...
    def login_func(self):
        if self.email_entry.get() == "" or self.password_entry.get() == "":
            ...
            try:
                ...
                cur.execute(
                    "select * from student_register where email=%s and password=%s",
                ...
                if row == None:
                    ...
                    messagebox.showinfo(
                        "Success", "Wellcome to the PySeek family", parent=self.window
                    )
    ...
    def change_pass(self):
        if (
            self.email_entry.get() == ""
            or self.sec_ques.get() == "Select"
            or self.new_pass.get() == ""
        ):
            ...
            try:
                ...
                cur = connection.cursor()
                cur.execute(
                    "select * from student_register where email=%s and question=%s and answer=%s",
                ...
                if row == None:
                    ...
                    try:
                        cur.execute(
                            "update student_register set password=%s where email=%s",
